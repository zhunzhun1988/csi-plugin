...
func (ns *nodeServer) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	podId := getPodIdFromMountTargetPath(targetPath)
	...
	if share == false && recycle == true {
		if err := ns.waitPodHostPathPVMountd(podId, 60); err != nil {
	...
	if err := ns.waitSyncToCache(podId, 3*time.Second); err != nil {
		return nil, fmt.Errorf("wait pod %s err:%v", podId, err)
	}
	if ok, path, err := ns.xfsquotamanager.AddQuotaPath(volumeId, podId, quotaSize, quotaSize, recycle); ok == false || err != nil {
		glog.V(1).Infof("AddQuotaPath volumeId:%s, podId:%s, quotaSize:%d, ok:%t, err:%v", volumeId, podId, quotaSize, ok, err)
		return nil, status.Error(codes.Internal, err.Error())
	} else {
		if path == "" {
			glog.V(1).Infof("AddQuotaPath volumeId:%s, podId:%s, quotaSize:%d, path is empty", volumeId, podId, quotaSize)
		...
		if err := ns.getMounter().Mount(path, targetPath, "", options); err != nil {
			...
			glog.V(1).Infof("AddQuotaPath volumeId:%s, podId:%s, quotaSize:%d, mount err:%v, umounterr:%v", volumeId, podId, quotaSize, err, umounterr)
			return nil, err
		}
		ns.addPathUsed(path, targetPath)
		ns.xfsquotamanager.AddQuotaPathMountPath(path, targetPath)
		glog.V(1).Infof("AddQuotaPath volumeId:%s, podId:%s, quotaSize:%d, mount %s success, usetime:%v", volumeId, podId, quotaSize, path, time.Since(starttime))
	}
...
func (ns *nodeServer) waitPodHostPathPVMountd(podId string, timeOut int) error {
	...
	return fmt.Errorf("wait pod %s hostpathpv mounted timeout")
}
	...
	if pod, err := ns.k8sClient.GetPodByPodId(podId); err != nil || pod == nil {
		...
		for _, podVolume := range pod.Spec.Volumes {
			if pvcSource := podVolume.VolumeSource.PersistentVolumeClaim; pvcSource != nil {
				...
				if ok := ns.isHostPathPVMountedByPod(string(pv.UID), podId); ok == false {
					return false
				}
...
func (ns *nodeServer) isHostPathPVMountedByPod(volumeId, podId string) bool {
